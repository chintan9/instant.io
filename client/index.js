...
global.WEBTORRENT_ANNOUNCE =
    createTorrent.announceList.map(function(arr) { return arr[0] })
        .filter(function(url) {
...
function init() {
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      e.preventDefault()
      downloadTorrent(
          document.querySelector('form input[name=torrentId]').value.trim())
    })
  }
  ...
  if ('registerProtocolHandler' in navigator) {
    navigator.registerProtocolHandler('magnet', window.location.origin + '#%s',
                                      'Instant.io')
  }
}
...
function getRtcConfig(cb) {
  ...
  get.concat({url : '/__rtcConfig__', timeout : 5000}, function(err, res,
                                                                data) {
...
function onFiles(files) {
  ...
  files.forEach(function(
      file) { debug(' - %s (%s bytes)', file.name, file.size) })
...
function isNotTorrentFile(file) { return !isTorrentFile(file) }

function downloadTorrent(torrentId) {
  var disallowed = DISALLOWED.some(function(
      infoHash) { return torrentId.indexOf(infoHash) >= 0 })
...
function downloadTorrentFile(file) {
  util.unsafeLog('Downloading torrent from <strong>' + escapeHtml(file.name) +
                 '</strong>')
  getClient(function(err, client) {
    if (err)
      return util.error(err)
...
function seed(files) {
  if (files.length === 0)
    return util.log('Seeding ' + files.length + ' files')
...
function onTorrent(torrent) {
  ...
  util.log('"' + torrentFileName + '" contains ' + torrent.files.length +
           ' files:')

  torrent.files.forEach(function(file) {
    util.unsafeLog('&nbsp;&nbsp;- ' + escapeHtml(file.name) + ' (' +
  ...
  util.log(
      ...
      '<a href="' + escapeHtml(torrent.magnetURI) +
      '" target="_blank">[Magnet URI]</a> ' +
      '<a href="' + escapeHtml(torrent.torrentFileBlobURL) +
      '" target="_blank" download="' + escapeHtml(torrentFileName) +
      '">[Download .torrent]</a>')
  ...
  function updateSpeed() {
    ...
    else {remaining = torrent.timeRemaining !== Infinity
