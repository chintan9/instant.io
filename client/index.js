...
function getRtcConfig(cb) {
  ...
  get.concat({ url: "/__rtcConfig__", timeout: 5000 }, function(
    err,
    res,
    data
  ) {
...
function downloadTorrentFile(file) {
  util.unsafeLog(
    "Downloading torrent from <strong>" + escapeHtml(file.name) + "</strong>"
  );
  getClient(function(err, client) {
    if (err) return util.error(err);
...
function seed(files) {
  if (files.length === 0) return util.log("Seeding " + files.length + " files");
